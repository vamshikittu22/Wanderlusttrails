...
file_put_contents(__DIR__ . "/../logs/debug.log", "createBlog API Started - Method: " . $_SERVER['REQUEST_METHOD'] . " at " . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    file_put_contents(__DIR__ . "/../logs/debug.log", "Received data: userId=$userId, title=$title, status=$status\n", FILE_APPEND);
    ...
    try {
        ...
        $result = $blogsModel->createBlog($userId, $title, $content, $mediaUrls, $status);

        file_put_contents(__DIR__ . "/../logs/debug.log", "createBlog result: " . json_encode($result) . "\n", FILE_APPEND);
        ...
        file_put_contents(__DIR__ . "/../logs/debug.log", "Exception in createBlog: " . $e->getMessage() . "\n", FILE_APPEND);
        http_response_code(500);
